...
bool ServiceUpdateCartTokomedia(struct UpdateCartTokomedia input) {
    ...
    sprintf(query, "UPDATE %s SET item_name = \'%s\', price_per_unit = %d, qty = %d, total_price = %d, created_at = \'%s\', itemlist_id = %d, itemlist_id = %d,tokomedia_shop_id = %d WHERE id = %d;", env.UserGetCartTokomediaTableName(), input.item_name, input.price_per_unit, input.qty, input.total_price, input.created_at, input.itemlist_id, input.tokomedia_user_id, input.tokomedia_shop_id, input.id);
