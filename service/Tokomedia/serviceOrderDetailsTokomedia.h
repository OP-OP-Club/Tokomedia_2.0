...
bool ServiceCreateOrderDetailsTokomedia(struct NewOrderDetailsTokomedia input){
    ...
    sprintf(query, "INSERT INTO %s (item_name, status, price_per_unit, qty, total_price, created_at, updated_at, itemlist_id, tokomedia_shop_id, transaction_id) VALUES (\'%s\', %d, %d, %d, %d, \'%s\', \'%s\', %d, %d, %d);", env.UserGetOrderDetailsTokomediaTableName(), input.item_name, input.price_per_unit, input.qty, input.total_price, input.created_at, input.itemlist_id, input.tokomedia_user_id, input.tokomedia_shop_id);
